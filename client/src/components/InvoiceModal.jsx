...
const InvoiceModal = ({ isOpen, onClose, onRefresh, clients, products, accounts, invoices, editData, initialType }) => {
  ...
  const handleProductSelect = (selectedOption) => {
    ...
    setFormData({
      ...
      items: [
        ...formData.items,
        {
          ...
          hsnCode: selectedOption.data.hsnCode || '',
          quantity: 1,
    ...
    if (selectedOption.data.hsnCode) {
      const newIdx = formData.items.length;
      setHsnQuery(p => ({ ...p, [newIdx]: selectedOption.data.hsnCode }));
    }
  };
    ...
    <div>
      ...
      <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500/30">
        ...
        <input
          type="text"
          readOnly
          className="flex-1 px-3 py-3 bg-transparent text-sm font-black outline-none text-slate-800 w-16 cursor-not-allowed"
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest w-36">HSN / SAC</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="relative">
                            <input
                              ...
                              placeholder="HSN..."
                              autoComplete="off"
                              className={`w-full p-2 border rounded-lg text-xs font-bold outline-none ${item.hsnCode ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200'}`}
                            />
                            {showHsnDrop[idx] && (hsnResults[idx] || []).length > 0 && (
                              <div className="absolute left-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-2xl max-h-48 overflow-y-auto" style={{zIndex: 9999, position: 'fixed', width: '380px', maxWidth: '90vw'}}>
                                {(hsnResults[idx] || []).map((h, i) => (
                                  <button key={i} type="button"
                                    onMouseDown={() => selectHsn(idx, h)}
                                    className="w-full text-left px-3 py-2 hover:bg-indigo-50 flex gap-2 items-start border-b border-slate-50 last:border-0">
                                    <span className="text-[10px] font-black text-indigo-600 w-12 shrink-0 mt-0.5">{h.code}</span>
                                    <span className="text-[10px] text-slate-600 font-medium leading-tight">{h.description}</span>
                                  </button>
                                ))}
                              </div>
                            )}
              ...
              <div className="w-full md:w-1/2 space-y-4">
                ...
                {!sellerState && (
                  <div className="text-[9px] text-amber-600 font-bold bg-amber-50 rounded-lg px-2 py-1">
                    ⚠ Set your business State in Settings to enable CGST/SGST/IGST split
                  </div>
                )}
