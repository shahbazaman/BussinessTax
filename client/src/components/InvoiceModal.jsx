...
const InvoiceModal = ({ isOpen, onClose, onRefresh, clients, products, accounts = [] }) => {
  ...
  const downloadReceipt = () => {
    ...
    // Fixed: changed doc.savedoc.save to doc.save
  ...
  const handleRazorpayPayment = async () => {
    ...
    if (!razorpayKey) {
      toast.info('💳 Razorpay keys not configured. Add VITE_RAZORPAY_KEY_ID to .env', { autoClose: 5000 });
      return;
    }
    if (!savedInvoiceId) {
      toast.warning('⚠️ Please save the invoice first.');
      return;
    }
    if (!selectedAccountId) {
      toast.warning('⚠️ Please select a receiving account.');
      return;
    }
    if (!window.Razorpay) {
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            ...
            setPaymentLoading(false);
            toast.error('⚠️ Verification failed. ID: ' + response.razorpay_payment_id);
          }
        },
        prefill: {
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
          ...
          <div>
            ...
            <div className="flex gap-2 mt-2">
              <button
                ...
                className={`px-4 py-1 rounded-full text-xs font-bold transition-all ${type === 'Sale' ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-500'}`}
              > SALES (INCOME) </button>
              <button
                type="button"
                onClick={() => setType('Purchase')}
                className={`px-4 py-1 rounded-full text-xs font-bold transition-all ${type === 'Purchase' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500'}`}
              > PURCHASE (EXPENSE) </button>
            </div>
          </div>
          <button type="button" onClick={handleClose} className="p-2 hover:bg-slate-200 rounded-full transition-all"><X /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Select {type === 'Sale' ? 'Client' : 'Vendor'}</label>
              <select
          ...
          <div className="relative">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Search Products</label>
            <div className="flex items-center bg-slate-100 rounded-2xl px-4 mt-1">
              <Search size={18} className="text-slate-400" />
              <input
                type="text"
                placeholder="Search rice, wheat..."
            ...
            {searchQuery && (
              <div className="absolute w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl z-10 max-h-48 overflow-y-auto">
                {searchResults.map((v) => (
                  <div key={v._id} onClick={() => handleAddItem(v)} className="p-4 hover:bg-blue-50 cursor-pointer flex justify-between items-center border-b border-slate-50">
                    <span className="font-bold text-slate-700">{v.displayLabel}</span>
                    <span className={`text-xs font-black ${v.stock <= 5 ? 'text-rose-500' : 'text-slate-400'}`}>Stock: {v.stock}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="space-y-3">
            {selectedItems.map((item, index) => (
              <div key={index} className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                ...
                />
                <button type="button" onClick={() => setSelectedItems(selectedItems.filter((_, i) => i !== index))} className="text-rose-500 p-2"><Trash2 size={18} /></button>
              </div>
            ))}
          </div>
          ...
          {type === 'Sale' && (
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase ml-2 flex items-center gap-1">
                <Landmark size={12} /> Receiving Account
              </label>
              <select
              ...
              >
                ...
                {accounts.map((acc) => <option key={acc._id} value={acc._id}>{acc.bankName} — {acc.accountType}</option>)}
              </select>
            </div>
          )}
          ...
          {savedInvoiceId && (
            <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl px-5 py-3">
              ...
              <div>
                ...
                <p className="text-slate-400 text-[10px] font-mono mt-0.5">ID: #{savedInvoiceId.slice(-10).toUpperCase()}</p>
              </div>
            </div>
          )}
          ...
          {paymentDone && (
            <div className="flex flex-col gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-5 py-4">
              ...
              <button onClick={downloadReceipt} className="bg-blue-600 text-white text-[10px] font-bold py-2 px-4 rounded-xl flex items-center justify-center gap-2 hover:bg-blue-700 transition-all">
        ...
        <div className="p-6 bg-slate-900 flex flex-col sm:flex-row justify-between items-center gap-4 mt-auto">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            {!savedInvoiceId && (
              <button onClick={handleSubmit} className="flex-1 sm:flex-none px-6 py-4 rounded-2xl font-black uppercase bg-emerald-500 text-white flex items-center justify-center gap-2 text-sm hover:bg-emerald-400">
                <ShoppingCart size={18} /> Confirm {type}
              </button>
            )}
            {savedInvoiceId && !paymentDone && type === 'Sale' && (
              <button onClick={handleRazorpayPayment} disabled={paymentLoading} className={`flex-1 sm:flex-none px-6 py-4 rounded-2xl font-black uppercase flex items-center justify-center gap-2 text-sm ${paymentLoading ? 'bg-blue-400 cursor-wait text-white' : 'bg-blue-600 hover:bg-blue-500 text-white'}`}>
                <CreditCard size={18} /> {paymentLoading ? 'Processing...' : 'Pay via Razorpay'}
              </button>
            )}
            <button onClick={handleClose} className="px-6 py-4 rounded-2xl font-black uppercase text-sm bg-slate-700 text-slate-300 hover:bg-slate-600">
