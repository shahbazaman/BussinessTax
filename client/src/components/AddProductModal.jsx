...
<div className="md:col-span-2 space-y-2">
  ...
  <div className="relative flex gap-2">
    <input
      ref={el => { if (el) window.__hsnProductRef = el; }}
      type="text"
      value={hsnQuery}
      ...
      placeholder="Type code (e.g. 8471) or description (e.g. computer, cotton)..."
      autoComplete="off"
      className={`flex-1 p-4 rounded-2xl border-2 outline-none font-bold text-slate-700 text-sm transition-all
        ${productData.hsnCode ? 'bg-indigo-50 border-indigo-200 text-indigo-800' : 'bg-slate-50 border-transparent focus:border-indigo-300 focus:bg-white'}`}
    />
    {productData.hsnCode && (
      <span className="flex items-center bg-indigo-500 text-white text-[10px] font-black px-4 rounded-2xl whitespace-nowrap shadow">
        {productData.hsnCode}
      </span>
    )}
  </div>
  {showHsnDropdown && hsnResults.length > 0 && ReactDOM.createPortal(
    <div
      style={{
        position: 'fixed',
        top: (() => { const el = window.__hsnProductRef; return el ? el.getBoundingClientRect().bottom + 4 : 200; })(),
        left: (() => { const el = window.__hsnProductRef; return el ? el.getBoundingClientRect().left : 100; })(),
        width: (() => { const el = window.__hsnProductRef; return el ? Math.max(el.getBoundingClientRect().width, 460) : 460; })(),
    ...
    >
      <div className="flex items-center px-5 py-2.5 bg-indigo-50 border-b border-slate-100">
        <span className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">
          {hsnResults.length} results for &quot;{hsnQuery}&quot;
        </span>
        <button type="button" onClick={() => setShowHsnDropdown(false)}
          className="ml-auto text-slate-400 hover:text-slate-700 text-xs font-black px-1">✕</button>
      </div>
      <div className="max-h-64 overflow-y-auto divide-y divide-slate-50">
        {hsnResults.map((item, i) => (
          <button key={i} type="button" onMouseDown={() => selectHsn(item)}
            className="w-full text-left px-5 py-3 hover:bg-indigo-50 flex items-start gap-3 transition-colors">
            <span className="text-[11px] font-black text-white bg-indigo-500 px-2 py-1 rounded-lg shrink-0 min-w-14 text-center">{item.code}</span>
            <span className="text-[11px] text-slate-700 font-medium leading-snug mt-0.5">{item.description}</span>
          </button>
        ))}
      </div>
    </div>,
  ...
  {showHsnDropdown && hsnResults.length === 0 && hsnQuery.length >= 2 && (
    <p className="text-xs font-bold text-slate-400 px-1 mt-1">No HSN/SAC code found for "<span className="text-indigo-600">{hsnQuery}</span>"</p>
  )}
</div>
