...
import { 
  ...
  X, Search, Filter, Loader2, Receipt, Landmark, Phone, Briefcase, MapPin, Fingerprint
} from 'lucide-react';
...
const Employees = () => {
  ...
  const initialState = { 
    name: '', role: '', dailyRate: '', 
    email: '', phone: '', contactNumber: '', // New field
    homeAddress: '', // New field
    verificationIdType: 'National ID', // New field (Dropdown)
    idNumber: '', // New field
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 bg-slate-50 min-h-screen">
      {/* Header & Stats (Identical to your provided code, omitted for brevity) */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-3">
            <button onClick={() => setShowModal(true)} className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 hover:bg-blue-600 transition-all shadow-xl shadow-slate-200">
              <UserPlus size={16} /> Add Employee
            </button>
        </div>
      </div>

      {/* Table Section (Using your logic) */}
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{emp.role} • {emp.verificationIdType}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-sm font-bold text-slate-500">{emp.phone}</td>
                  <td className="px-8 py-5 flex justify-end gap-2">
                      <button onClick={() => handleMarkAttendance(emp._id)} disabled={isAlreadyMarked(emp.lastAttendanceDate)} className={`p-3 rounded-xl ${isAlreadyMarked(emp.lastAttendanceDate) ? 'text-slate-200' : 'text-emerald-600 hover:bg-emerald-50'}`}><CalendarCheck size={18} /></button>
                      <button onClick={() => openEditModal(emp)} className="p-3 text-slate-400 hover:bg-slate-900 hover:text-white rounded-xl"><Edit2 size={18} /></button>
                      <button onClick={() => handleDelete(emp._id)} className="p-3 text-rose-500 hover:bg-rose-50 rounded-xl"><Trash2 size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[75vh] overflow-y-auto no-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input required className="w-full p-4 bg-slate-50 rounded-2xl border-none outline-none font-bold text-sm" placeholder="+1 234..." value={formData.contactNumber} onChange={e => setFormData({...formData, contactNumber: e.target.value})} />
                </div>
                <div className="space-y-1">
                  ...
                  <input required className="w-full p-4 bg-slate-50 rounded-2xl border-none outline-none font-bold text-sm" placeholder="Street, City, Zip" value={formData.homeAddress} onChange={e => setFormData({...formData, homeAddress: e.target.value})} />
                </div>
              </div>
              ...
              <div className="p-6 bg-blue-50/50 rounded-4xl border border-blue-100 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <select 
                    className="w-full p-4 bg-white rounded-2xl border-none outline-none font-bold text-sm text-slate-600"
                  ...
                  <input required placeholder="ID Number (e.g. PASS1234)" className="w-full p-4 bg-white rounded-2xl border-none outline-none font-bold text-sm text-slate-800" value={formData.idNumber} onChange={e => setFormData({...formData, idNumber: e.target.value})} />
                </div>
              </div>
              ...
              <div className="p-6 bg-slate-900 rounded-4xl space-y-4 shadow-lg shadow-slate-200">
                <div className="flex items-center gap-2 text-slate-400 mb-2">
                  <Landmark size={14}/>
                  <span className="text-[10px] font-black uppercase tracking-widest">Settlement (Bank Account)</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
