...
export const createInvoice = async (req, res) => {
  try {
    const { client, items, discount, type, invoiceDate, status, notes, globalTaxRate, gstNumber, billingAddress, shippingAddress, invoiceNumber, purchaseNumber, referenceNumber } = req.body;
    ...
    const invoice = new Invoice({
      user: req.user._id, client, invoiceNumber, purchaseNumber, referenceNumber,
      invoiceDate: invoiceDate || new Date(), gstNumber, billingAddress, shippingAddress,
      items: validatedItems, discount: Number(discount || 0),
      globalTaxRate: Number(globalTaxRate || 0), status: status || 'Pending',
      type: invoiceType, notes
    });

    const savedInvoice = await invoice.save();

    // Adjusted Stock Update
...
export const updateInvoice = async (req, res) => {
  try {
    ...
    for (const item of oldInvoice.items) {
      ...
      await Product.updateOne({ _id: item.productId, "variants._id": item.variantId }, { $inc: { "variants.$.stock": revertQty } });
    }

    // 2. Update Invoice
    const { items, client, ...updateData } = req.body;
    Object.assign(oldInvoice, { ...updateData, client, items });
    ...
    for (const item of updatedInvoice.items) {
      ...
      const result = await Product.updateOne({ _id: item.productId, "variants._id": item.variantId }, { $inc: { "variants.$.stock": newAdjustment } });
